...
gpuMaxParallelism = str(sys.argv[5])

inFileName = os.path.basename(inFilePath).split('.')[0]
outFileName = inFileName + "T" + timeoutSeconds + "W" + mddWidth + "P" + gpuMaxParallelism + ".csv"
outFile = open(outFileName, "w")

for mainQueueMaxSize in mainQueueMaxSizes:
    for mainQueueReducedSize in mainQueueReducedSizes:
        cmd = [binPath] + [inFilePath] + [timeoutSeconds] + [str(mainQueueMaxSize)] + [str(mainQueueReducedSize)] + [mddWidth] + [gpuMaxParallelism]
        ...
        print(value)
        info = value + "," + str(mainQueueMaxSize) + "," + str(mainQueueReducedSize)
        outFile.write(info + "\n")
